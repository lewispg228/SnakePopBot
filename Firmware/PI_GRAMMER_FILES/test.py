...
def clean_results():
        ...
        f.truncate()
        f.close()
        f = open('/home/pi/verify_fuse_lock_results.txt', 'w')
        f.truncate()
        f.close()        
...
def program_verify_fuse_lock():
        #GPIO.setup(PGM_SWITCH, GPIO.IN)
	GPIO.output(PGM_SWITCH, GPIO.HIGH)
	time.sleep(.1)        
        clean_results()
        command = "sh /home/pi/pi_program_verify_fuse_lock.sh"
        import subprocess
        process = subprocess.Popen(command.split(), stdout=subprocess.PIPE)
        output = process.communicate()[0]
        print output
...
def parse_results_verify_fuse_lock():
        #variables to store success/failure of each programming step
        hfuse = False
        lfuse = False
        efuse = False
        lock = False
        f = open('/home/pi/verify_fuse_lock_results.txt', 'r')
        for line in f:
                if 'avrdude: 1 bytes of hfuse verified' in line:
                        print line
                        hfuse = True
        ...
        if(lock == True):
                ...
                GPIO.output(SERIAL_F, GPIO.HIGH)
        time.sleep(1) # led leds stay on for 3 secs
        all_leds_off()
...
while True:
        ...
        if GPIO.input(CAPSENSE) == True:
                ...
                # program_verify_fuse_lock()
                # parse_results_verify_fuse_lock()
                program()
                parse_results()
